...
from hmasd.envs.alice_and_bob.alice_and_bob0 import Alice_and_Bob as Alice_and_Bob0
from hmasd.envs.env_wrappers import ShareSubprocVecEnv, ShareDummyVecEnv
from hmasd.runner.shared.alice_and_bob_runner import AliceBobRunner as Runner
...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            if all_args.env_name == "alice_and_bob":
                if all_args.game_version == 0:
                    env = Alice_and_Bob0()
            else:
...
def parse_args(args, parser):
    ...
    parser.add_argument('--game_version', type=int, default=0)
